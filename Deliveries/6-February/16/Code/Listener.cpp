...
using namespace adevs;
	...
	void Listener::outputEvent(Event<Loan> x, double t)
		...
		outputfile << x.value.get_IDborrower()
				...
				<< "," << x.value.getType();
		outputfile << "," ; if((x.value.getType()==1) || (x.value.getType()==11) ) outputfile  << x.value.get_IDborrower() ; else outputfile << "";

		outputfile << "," ; if((x.value.getType()==2) || (x.value.getType()==22)) outputfile  << x.value.get_IDlender() ; else outputfile << "";

		outputfile << "," ; if(x.value.getType()==11) outputfile  << x.value.GetLinkedAgentID() ; else outputfile << "";

		outputfile << "," ; if(x.value.getType()==22) outputfile  << x.value.GetLinkedAgentID(); else outputfile << "";

...
Generator::Generator() :
		adevs::Atomic<Loan>() { //n_banks=1,
	set_time_next_deposit();
	set_time_next_loan();
	nextl = 0;
	nextd = 0;
}
...
void Generator::delta_int() {
	if (time_next_deposit <= time_next_loan) {
		time_next_loan -= time_next_deposit;
		set_time_next_deposit();
		nextd++;
	} else {
		time_next_deposit -= time_next_loan;
		set_time_next_loan();
		nextl++;
	}
}
...
void Generator::output_func(adevs::Bag<Loan>& yb) {
	...
	if (time_next_deposit <= time_next_loan) {
		...
		assert(BNetwork::getBankCount()==2);
...
unsigned int Generator::get_number_of_deposits() {
	return nextd;
}

unsigned int Generator::get_number_of_loans() {
	return nextl;
}

void Generator::set_time_next_deposit() {
	time_next_deposit = rgen.exponential(
			1.0 / (Df * (double) BNetwork::getBankCount()));
}

void Generator::set_time_next_loan() {
	time_next_loan = rgen.exponential(
			1.0 / (Lf * (double) BNetwork::getBankCount()));
}

Bank* Generator::AssignBank(int n) {
cerr << (int) (n * (rand() / (RAND_MAX + 1.0))) << endl;

...
//		static int GetCLoanRate();
		...
		void SetIBDepositRate(double  r);
		//set rate asked for IB loans
		void SetIBLoanRate(double  r);

		double  GetIBDepositRate();

		double  GetIBLoanRate();
	...
	private:
		...
		//Backlogs
		std::map< double , std::deque<Loan*> > UMLoans;

		std::map< double  , std::deque<Loan*> > UMDeposits;
        ...
        static bool bestPayoff ( const Action& a1 , const Action &a2) ;
...
//		static std::map<vector<int>,int > h_data;
        ...
        std::map<State,vector<Action>> stradep ;
        std::map<State,vector<Action>> straloan ; 

...
//set deposit offered for IB deposits
	...
	static void updateIlliquidAssetPrice(double t);
...
private:
	...
	static double illiquidAssetPrice;

	int totalH;

	static std::set<State*> stateSet;

	static std::set<Strategy*> strategySet;
...
//	std::ofstream log;
	...
	// Map with value associated to matched IB loan ID
	std::map<double, int> valueMatchedIBLoans;

	void removeFromVM(int ID);
	...
	static adevs::rv def;
	static adevs::rv a;
	static adevs::rv centralBank;
	static adevs::rv randomMatch;
	static adevs::rv tremblingRandomVariable;
	static adevs::rv illiquidAssetPriceProcess;
	unsigned int defaults;
	double Pdefault;
	// Function for sorting deposits or loans
...
//		static std::map<vector<int>,int > h_data;
	...
	void SetInitialValue(const State& state, const Action& action, double v);

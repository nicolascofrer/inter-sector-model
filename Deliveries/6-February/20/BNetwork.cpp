		...
		Network<Loan>() // call the parent constructor
{
	...
	Bank::SetCBLoanRate(2 * pRatesLevels);
	...
	for (int i = 0; i != NBANKS; i++) {
		for (int j = (i + 1), n = 0;
			...
			if (Banks[j % ((int)NBANKS)] != Banks[i]) { //Check if banks can be partners

				Banks[i]->AddPartner(Banks[j % ((int)NBANKS)]);
...
//			cout << "Bank " << (*iter)->getID() << " is insolvent " << endl;
...
//			//iter = Banks.erase(iter); // This should be better
			//Remove from map of active banks
			Banks.erase(iter);

			add_Bank(pDefBankruptBank);
			// I want to ensure that the iterator is valid once we have deleted an object from it
			iter = Banks.begin();
...
//			cout << "Canceled loan with ID " << (*iter)->getIDCanceledLoan()<<" by " << (*iter)->getID()<< " on transition method" << endl;
	...
	iter = Banks.begin();
		while (iter != Banks.end()) {
			if((*iter)->isRejectingIBLRequest() && (!IsBankrupt(getBankIncludingBankrupt((*iter)->getSenderCanceledRequest()))))
...
bool BNetwork::IsBankrupt(Bank* bank) {
	...
	return find(Banks.begin(), Banks.end(), bank) == Banks.end();
}

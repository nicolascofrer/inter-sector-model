                ...
                switch (qt.front().getType()) {
                    case 1:{
                        ...
                        if (l!=NULL){
                            ...
                            if (l->getType()==2){
                            actualize_equity (l->getInterestRate()-qt.front().getInterestRate());
                            if(UpdatingProcess){qt.front().updateAction(l->getInterestRate()-qt.front().getInterestRate());}
                            }
                    
                            if (l->getType()==22 ) {
                            actualize_equity(l->getInterestRate()-qt.front().getInterestRate()-Clink);
                            if(UpdatingProcess) {qt.front().updateAction(l->getInterestRate()-qt.front().getInterestRate()-Clink);}
                            }
                    
                        }
                        else{ actualize_equity(CBDFR-qt.front().getInterestRate());
                    ...
                    case 2 :
                    {
                // if( WillItDefault() ) { actualize_equity( ( qt.front()).getInterestRate() );
               
                    if (l!=NULL) {
                   
                       if (l->getType()==1){
                           actualize_equity (qt.front().getInterestRate()-l->getInterestRate());
                           if(UpdatingProcess){qt.front().updateAction(qt.front().getInterestRate()-l->getInterestRate());}
                       ...
                       if (l->getType()==11){
                           actualize_equity (qt.front().getInterestRate()-l->getInterestRate()-Clink);
                           if(UpdatingProcess){qt.front().updateAction(qt.front().getInterestRate()-l->getInterestRate()-Clink);}
                       } else {
                            actualize_equity(CBDFR-qt.front().getInterestRate());
                           if(UpdatingProcess){ qt.front().updateAction(CBDFR-qt.front().getInterestRate());}
				...
				if( ( (*iter).getType() == 1 ) | ( (*iter).getType() == 11) ) {
                    ...
                    else
                    {
                       ...
                       int  n=(int)((search->second).size());
                         ...
                         (*iter).setAction((*itera));
                         (*iter).SetdesiredRate((*itera).desiredRate);
                        // ça prend le premier element , attention aussi
                        }
                     ...
                     -I made a tiny change in the codes , we need to store equity , deposit , interest rate ....as double
                        
                        -at this step , all state are associate  to potential action with initial payoff ( expected maturity ).
                        
                        I want to choose the action with higher payoff , with probability 1-e   and choice a random action with probability e  (  where e is trembling probability ) :  this is my problem
                        
                        in delta int ()  , at the maturity , update different transaction with  the Dan's proposition.
                        
                        the code will remain almost the same, we just have to adapt delta int () and delta ext ()
                    */
...
//			Equities = aux;
//		}
            void Bank::actualize_equity ( double e ){ equity+=e ;}
...
//
		...
		void Bank::updateState(int type, Loan* loan){
			...
			if(UMDeposits.empty()) IBLoanRate = CBDFR;

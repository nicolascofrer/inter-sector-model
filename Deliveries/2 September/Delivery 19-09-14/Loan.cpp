...
bool Strategy::operator < (Strategy s) const{ return state < s.state & action < s.action;}
	...
	void Loan::SetLinkedAgent(Agent* b){linked = b; IDlinked = BNetwork::getBank(b);}
	...
	void Loan::SetSenderAgent(Agent* b){requestSender = b; IDrequestSender= BNetwork::getBank(b);}
	...
	void Loan::set_borrower(Agent* b) {borrower = b; IDborrower = BNetwork::getBank(b);}

	void Loan::set_borrower(int b){IDborrower = b; borrower = BNetwork::getBank(b);}
	...
	void Loan::set_lender(Agent* l) {lender = l; IDlender = BNetwork::getBank(l);}
...
//DEPOSITOR
	Depositor::Depositor(): Loan::Loan(){quantity = 1; type = 1; set_time_to_maturity(); expired = false; interest_rate = 0;}
	void Depositor::set_time_to_maturity(){t_to_maturity = d.exponential( DepositorMaturityParameter); duration = t_to_maturity;}

//BORROWER
	Borrower::Borrower(): Loan::Loan(){quantity = 1; type = 2; set_time_to_maturity(); expired = false; interest_rate = 0;}

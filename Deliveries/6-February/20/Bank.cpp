...
std::ofstream Bank::statesFile("statesFile.txt");
...
//	std::vector<Strategy*>::iterator iter3 = strategyBag.begin();
//	for (; iter3 != strategyBag.end(); iter3++)
//		delete *iter3;
...
//			cout << "Case 1" << endl;
			...
			CSH += cumulativeObjectValue[qt.front().getID()];
...
//							&& qt.front().GetSenderAgentID() != ID);
			...
			if (BNetwork::IsBankrupt(
					BNetwork::getBankIncludingBankrupt(
							qt.front().GetSenderAgentID()))
					&& !BNetwork::wasIBLiabilityPaid(qt.front().getID())) {
...
// Put new events into the queue, here we can to separate loan objects according to the type in the loan object
	...
	for (; iter != xb.end(); iter++) //We have to look at each new object coming
		...
		if (a.uniform(0, 1) < probTrembling) {
...
//			assert(*tempState==LastState);
			...
			std::set<State>::iterator setIt = stateSet.find(LastState);

			if (setIt == stateSet.end()) {
...
//				if (h[LastState].find(*iter2) == h[LastState].end()) {
				...
				LastState.setStateNumber(stateSet.size());
				statesFile << LastState.getStateNumber() << endl;
				stateSet.insert(LastState);

			} else {
				statesFile << (*setIt).getStateNumber() << endl;
...
//			strategyBag.push_back(tempStrategy);
			// Save strategy
...
// This is only the net amount of IB assets plus cash
double Bank::GetEquity() {
	return GetAssets() - qida - qilr -qd;
}
...
void Bank::SetInitialValue(const State& state, const Action& action,
		const double& v) {
...
void Bank::SendRating() {
	...
	if (((CSH + qila) / (qd + qida + qcblf) < DowngradeRatio1)
			| (GetEquity() / (CSH + ql + qila) < DowngradeRatio2))
...
//		assert(value>0);
		...
		assert(Rates.size()==pRatesLevels*2+1);
		assert(l.getInterestRate() >=0 && l.getInterestRate() <=pRatesLevels*2);

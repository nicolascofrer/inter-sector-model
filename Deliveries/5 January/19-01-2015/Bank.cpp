...
void Bank::UpdateW(Strategy* S, double CurrentV) {
	...
	log << "Updating continuation value" << endl;
	...
	variablesDebugging << time << "," << ValueFunction2[state][action] << ","
			<< totalH << endl;
}
...
void Bank::SetLqBnch() {
	LqBnch = beta2 * (qd + (qida + qilr) + qcblf); //This is rounded to int precision
}

bool Bank::CheckCashReserveRequirements() {
	return CSH > beta1 * (qd + (qida + qilr) + qcblf);
}
...
void Bank::resolveCanceledLoan(int IDCanceledLoan) {
	...
	cout << "RESOLVE CANCELED Bank " << ID << " Loan " << IDCanceledLoan
			<< endl;
...
// While the CSH requirement is not
	while (!CheckCashReserveRequirements() && (ql + (qila + qidr)) > 0)

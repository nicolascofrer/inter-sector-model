...
//	networkOutputfile << "Partners Bank " << NMAX << endl;
	// This is the change in the output file, no dependency on the number of banks
	generalInfo << "Time,";
	generalInfo << "Bank ID,";
	generalInfo << "Cash,";
...
//	generalInfo << "IB Deposits,";
	...
	generalInfo << "NetIBLiabilities,";

	generalInfo << "Illiquid asset price,";


	generalInfo << "CB Lending Facility,";
	generalInfo << "Equity,";
	generalInfo << "Visible Partners" << ",";
	generalInfo << "Total H" << ",";
	generalInfo << "Number of states" << ",";
	generalInfo << "Last Value Function" << ",";
	generalInfo << "Rd" << ",";
	generalInfo << "Rl" << endl;
...
//	equityOutputfile << "Equity Bank " << NMAX << endl;
	cout << "Cpp program Version 03-02-15" << endl;
...
//		}
		...
		//generator->set_n_banks( bnetwork->getBankCount() );
		Bank::updateIlliquidAssetPrice(sim->nextEventTime());
		Bank::setTime(sim->nextEventTime());
		sim->execNextEvent();

		assert(BNetwork::getBankCount()==NBANKS);

		for (int i = 0; i != BNetwork::getBankCount(); i++) {
			generalInfo << BNetwork::getBank(i)->printOutput();
		}

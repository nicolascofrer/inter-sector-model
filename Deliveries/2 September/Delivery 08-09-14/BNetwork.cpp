	...
	Network<Loan>() // call the parent constructor
{
	...
	Commercial_bank::SetDepositRateRange(0,0.1);
	Commercial_bank::SetLoanRateRange(0,0.1);
	...
	for (int i = 0; i != NBANKS-1 ; i++){
			for(int j=(i+1), n = 0; ( commercial_banks[i]->GetNPartners() < commercial_banks[i]->GetAllowedNPartners() ) & ( n!=NBANKS-1) ; j++, n++){
				if(( commercial_banks[j%(NBANKS)]->GetNPartners() < commercial_banks[j%(NBANKS)]->GetAllowedNPartners() ) & ( commercial_banks[j%(NBANKS)] !=commercial_banks[i] ) )  { //Check if banks can be partners

...
bool Bank::model_transition() {
	...
	if (checkInsolvent() | cancelingLoan)
		log << "Transition activated " << ID << endl;
	return checkInsolvent() | cancelingLoan;
}
...
//		//					8) partners bid
	...
	map<int, std::vector<Bank*> >::iterator iterMap;
...
void Bank::resolveCanceledLoan(int IDCanceledLoan) {
	deque<Loan>::iterator iterator_qt;

	for (iterator_qt = qt.begin(); iterator_qt != qt.end(); iterator_qt++)
		if ((*iterator_qt).getID() == IDCanceledLoan) {
			std::map<Loan*, Loan*>::iterator it1 = matchMap1.find(
					&(*iterator_qt));

			std::map<Loan*, Loan*>::iterator it2 = matchMap2.find(
					&(*iterator_qt));

			std::map<int, deque<Loan*> >::iterator itBacklogDeposits =
					UMDeposits.find((*iterator_qt).getDesiredMatchingRate());

			std::map<int, deque<Loan*> >::iterator itBacklogLoans =
					UMLoans.find((*iterator_qt).getDesiredMatchingRate());

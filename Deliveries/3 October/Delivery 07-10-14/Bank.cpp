...
//				if((qt.front()).getType() == 11 & (qt.front()).get_borrower()==this ) throw invalid_argument("self IB deposit request when not sending request");
		...
		void Bank::delta_ext(double e, const adevs::Bag<Loan>& xb)
		{
			...
			if(PartnersVector.size()>3) log << "Bank " << ID << " Partners set with more than 3 partners" << endl;
								...
								if(iter3 != UMLoans.rend()){if( ( (ValueFunction[LastState]).rbegin() -> second.desiredMatchingInterestRate <= (*iter3).first ) & ( (*iter3).second.size()!=0 )   ) {
										...
										//If rate are the same in the own backlog and in the interbank market, flip a coin
											...
											// match with interbank deposit, send request
												...
												assert(!BNetwork::IsBankrupt(PartnersVectorDR.rbegin()->second));
											...
											if(iter3 != UMDeposits.end()){if( ( (ValueFunction[LastState]).begin() -> second.desiredMatchingInterestRate >= (*iter3).first ) & ( (*iter3).second.size()!=0 )   ) {
													...
													//If rate are the same in the own backlog and in the interbank market, flip a coin
														...
														// match with interbank deposit, send request
															...
															assert(!BNetwork::IsBankrupt(PartnersVectorLR.begin()->second));
...
//				if( qilrQueue.back() == NULL ) throw invalid_argument("NULL POINTER");
	//			log << qilrQueue.back() << endl;
		...
		unsigned int Bank::getIBLoanRequestQueueSize(){ return qilr;}

		unsigned int Bank::getIBLoanAcceptedQueueSize(){ return qila;}
...
//		}
		...
		int Bank::getCBLendingFacility(){ return qcblf;}

		int Bank::getCash(){ return CSH;}
		...
		if (it == PartnersVector.end())	{
			log << "NEW PARTNER in addPartner" << endl;
		...
		PartnersVectorLRderef[ p ]= p->GetIBDepositRate();
			...
			log << "UPDATING INFO in addPartner" << endl;
		//Remove old info
	...
	//	log << "end addPartner " << endl;
		...
		void Bank::UpdatePartnersSet(int a){
			...
			if( a == 1 ){
				...
				//log << "BANK " << ID << " Partner Deleted size is " << PartnersVector.size() << endl;
				...
				while( PartnersVector.size()< min ( partners, (BNetwork::getBankCount()-1)) ){ aux = BNetwork::getBank( (int)(NBANKS*(rand()/(RAND_MAX+1.0))) ); if( (aux!= this) && (!BNetwork::IsBankrupt(aux)) ) AddPartner(aux);}
				//PartnersVector.erase ( find(PartnersVector.begin(), PartnersVector.end(), (PartnersVectorDR.begin())->second) );

				//log << "BANK " << ID << " Partner Added size is " << PartnersVector.size() << endl;
		...
		void Bank::UpdatePartnerInfo(){
			...
			set<Bank*>::iterator iter = PartnersVector.begin();
				...
				assert(find(PartnersVector.begin(),PartnersVector.end(),this)==PartnersVector.end());
...
//		//					10) your liquidity benchmark (only above or below).
		...
		void Bank::Liquidate(){tleft = DBL_MAX; PartnersVector.clear();equity=0; CSH = 0;}
		...
		void Bank::ReplaceBankruptPartner(Bank* bank){
				...
				//log << "REPLACE BANKRUPT PARTNER IS " << bank->getID() << endl;
				assert(bank != NULL);
							Bank* aux ;
							//int n = 0;
							...
							while( PartnersVector.size() < min ( partners, (BNetwork::getBankCount()-1)) ){
								...
								if((aux!= this) && (!BNetwork::IsBankrupt(aux)) && PartnersVector.find(aux) == PartnersVector.end() ){
									AddPartner(aux);
								}
		...
		void Bank::askCentralBank(){
			...
			if(centralBank.uniform(0,1)< 1-phiCB) CSH++;
		}

...
// Put new events into the queue, here we can to separate loan objects according to the type in the loan object
	...
	for (; iter != xb.end(); iter++) //We have to look at each new object coming
		...
		// Trembling
			...
			cout << "Bank " << ID << " trembled when using its optimal rule, using strategy in position " << (int)(actionsVector.size()) << endl;
			isTrembling=true;
			}
		...
		if (((*iter).getType() == 1) | ((*iter).getType() == 11)) {
			...
			if(isTrembling)
				LastAction= *actionsVector[(int)(actionsVector.size()*(rand()/(RAND_MAX+1.0)))];

			else LastAction = ValueFunction[LastState].rbegin()->second; // This is the best action the bank could take according to its value function
			// Save strategy
...
void Bank::UpdateW(Strategy* S, double CurrentV) {
	...
	int nh = S->getH();
	// we are using n%100 in the updating
	ValueFunction2[state][action] = ValueFunction2[state][action] * (nh % 100)
			/ ((nh % 100) + 1) + CurrentV / ((nh % 100) + 1);
	...
	for (; it != loanStrategyMap.end(); ++it) {
		totalH += (*it).second->getH();
		WdV += ((*it).second->getH()) * ((*it).second->getdV());
		maxH = max(maxH, (*it).second->getH());
	}
	WdV = WdV / totalH;

	cout << "Weighted dV = " << WdV
			<< " Most frequent strategy has frequency = " << maxH
			<< " Total H = " << totalH << " Size Strategy Map = "
			<< loanStrategyMap.size() << endl;

	probTrembling=min(abs(WdV)*probTrembling,probTrembling);
}
...
void Bank::updateCumulativeValueObject(Loan l, bool isThereDefault) {
	...
	if (l.wasMatched()) {
		...
		if (itInCumulativeObjectValueMap1 != cumulativeObjectValue.end()) {
			cumulativeObjectValue[MatchCouterpart.getID()] += valueCounterpart;
		} else
			cumulativeObjectValue[MatchCouterpart.getID()] = valueCounterpart;
		...
		// Actual value
		value = (1 - isThereDefault) * isADeposit * dtl
				* (Rates[CBDFR] - Rates[l.getInterestRate()])
				- isThereDefault * (1 + dtl * Rates[CBDFR])
				- l.FromIB() * linkcost;

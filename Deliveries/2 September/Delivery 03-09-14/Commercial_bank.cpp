...
using namespace std;
...

		{
			...
			//In case of deposit/////////////////////////////////////////////////////////////////////
			if((qt.front()).getID()==1) {reduce_equity( (qt.front()).GetQuantity()); qt.front().SetAsExpired(); //what is the amount lent?
			...
			if((qt.front()).getID()==2){ if( WillItDefault() ) {reduce_equity( ( qt.front()).GetQuantity() );  qt.front().SetAsExpired(); //what is the amount lent?
			...
			if(UpdatingProcess) UpdateW(LastState, LastAction, ( qt.front()).GetQuantity() );  qt.front().SetAsExpired();//Update given realized state is a successful loan
			}
			}
...
//					}
					...
					// If Output is a request for interbank deposit or loan, with a partner
					if(qt.front().getID()==11)//Send deposit request to partner
					{//Look for the highest deposit rate offered by partners, at rbegin()
						qt.front().SetLinkedAgent( (((PartnersVectorLR.rbegin())->second).rbegin())->second ); //This is the partner that should receive the request
...
//					else h[LastState][LastAction]+=1;
		...
		void Commercial_bank::output_func(adevs::Bag<Loan>& yb)
		{
			...
			// If Output is an expired deposit or loan that was not "used" (not matched, id=1 or id=2)
//			if(qt.front().getID()==1 | qt.front().getID()==2)
			// Send deposit or loan request to partner

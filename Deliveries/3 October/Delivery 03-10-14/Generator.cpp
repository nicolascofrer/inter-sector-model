...
#include "Parameters.h"
		...
		void Generator::output_func(adevs::Bag<Loan>& yb){
			...
			l->set_borrower( AssignBank (BNetwork::getBankCount()));
			//l->SetLinkedAgent(0);
			l->setID(nextd);
			yb.insert( *l );
			...
			Loan* l = new Borrower();
			l->set_lender(  AssignBank (BNetwork::getBankCount()));
			//l->SetLinkedAgent(0);
			l->setID(nextl);
			yb.insert( *l );
			}
		...
		Bank* Generator::AssignBank(int n){
			return BNetwork::getBank( (int) rand()%n );  //Only for testing, this should be a truly uniform distributed int, std::uniform_int_distribution maybe?
}
